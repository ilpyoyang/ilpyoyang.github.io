...
##### Dynamic IAM Policy
정책상 동적으로 IAM을 변경하기 위해서는 `${aws:username}`으로 설정이 가능합니다.
```
{
  "Version": "2012-10-17",
  "Statement": [{
...
##### PassRole
사용자가 모든 EC2 인스턴스를 실행할 권한을 부여하며, 특정 IAM 역할(`Get-pics`)을 EC2 인스턴스에 할당할 수 있는 권한을 부여(`iam:PassRole`)합니다.
```
{
  "Version": "2012-10-17",
  "Statement": [
...
AWS 리소스에 대한 액세스를 제어할 수 있는 임시 보안 자격 증명을 생성해서 사용자에게 제공합니다. 임시적이고, 사용자에게 권한이 있다면 신규 보안 자격을 요청할 수 있습니다. `https://sts.amazonaws.com` 단일 엔드포인트가 있는 전역적 서비스입니다.
- `AssumeRole` - 계정이나 상호 계정에서 역할을 수행
- `AssumeRoleWithSAML` - 사용자가 일시적으로 SAML에 로그인 했을 때 주어지는 자격증명
- `AssumeRoleWithWebIdentity` - ID 제공자에게 로그인한 사용자 역할을 반환, 지금은 Cognito Identity Pool로 대체 이용
- `GetSessionToken` - MFA를 위한 토큰 획득
  - `"aws:MultiFactorAuthPresent" : "true"`
- `DecodeAuthorizationMessage` - AWS API 에러가 발생했을 때 메시지 암호화
